...
insert_read_all() ->
    ...
    io:format(" 1", []),
    {ok,1} = db_rec:read(curt, Db0),

    io:format(" 2", []),
    {ok,2} = db_rec:read(bert, Db0),

    io:format(" 3", []),
    {ok,3} = db_rec:read(sune, Db0),

    io:format(" 4", []),			%Read non-existent key
    {error,instance} = db_rec:read(sten, Db0),
...
insert_delete_one() ->
    ...
    io:format(" 1", []),
    {ok,1} = db_rec:read(curt, Db0),
    {error,instance} = db_rec:read(curt, db_rec:delete(curt, Db0)),

    io:format(" 2", []),
    {ok,2} = db_rec:read(bert, Db0),
    {error,instance} = db_rec:read(bert, db_rec:delete(bert, Db0)),

    io:format(" 3", []),
    {ok,3} = db_rec:read(sune, Db0),
    {error,instance} = db_rec:read(sune, db_rec:delete(sune, Db0)),

    io:format(" 4", []),			%Delete non-existent key
    {error,instance} = db_rec:read(sten, Db0),
    {error,instance} = db_rec:read(sten, db_rec:delete(sten, Db0)),
...
insert_delete_all() ->
    ...
    io:format(" 1", []),
    Empty = lists:foldl(fun (K, D) -> db_rec:delete(K, D) end,
			Db0, [curt,bert,sune]),
    io:format(" 2", []),
    Empty = lists:foldl(fun (K, D) -> db_rec:delete(K, D) end,
			Db0, [bert,sune,curt]),
...
insert_overwrite_one() ->
    ...
    io:format(" 1", []),
    {ok,1} = db_rec:read(curt, Db0),
    {ok,10} = db_rec:read(curt, db_rec:write(curt, 10, Db0)),

    io:format(" 2", []),
    {ok,2} = db_rec:read(bert, Db0),
    {ok,20} = db_rec:read(bert, db_rec:write(bert, 20, Db0)),

    io:format(" 3", []),
    {ok,3} = db_rec:read(sune, Db0),
    {ok,30} = db_rec:read(sune, db_rec:write(sune, 30, Db0)),
...
insert_overwrite_all() ->
    ...
    io:format(" 1", []),
    {ok,10} = db_rec:read(curt, Db1),

    io:format(" 2", []),
    {ok,20} = db_rec:read(bert, Db1),

    io:format(" 3", []),
    {ok,30} = db_rec:read(sune, Db1),

    io:format(" 4", []),			%Read non-existent key
    {error,instance} = db_rec:read(sten, Db1),
    io:format(" - ok\n").

%% init_db([{Key,Val}]) -> Database.
%%  Create and initialise a database with keys curt, bert and sune.

init_db(Vals) -> write_db(Vals, db_rec:new()).

%% write_db([{Key,Val}], Database) -> Database.
%%  Writes in all key-val into existing database.

write_db(Vals, Db) ->
    lists:foldl(fun ({K,V}, D) -> db_rec:write(K, V, D) end, Db, Vals).
