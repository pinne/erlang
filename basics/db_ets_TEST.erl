...
insert_read_all() ->
    ...
    io:format(" 1", []),
    {ok,1} = db_ets:read(curt, Db0),

    io:format(" 2", []),
    {ok,2} = db_ets:read(bert, Db0),

    io:format(" 3", []),
    {ok,3} = db_ets:read(sune, Db0),

    io:format(" 4", []),			%Read non-existent key
    {error,instance} = db_ets:read(sten, Db0),
...
insert_delete_one() ->
    ...
    io:format(" 1", []),
    {ok,1} = db_ets:read(curt, Db0),
    {error,instance} = db_ets:read(curt, db_ets:delete(curt, Db0)),

    io:format(" 2", []),
    {ok,2} = db_ets:read(bert, Db0),
    {error,instance} = db_ets:read(bert, db_ets:delete(bert, Db0)),

    io:format(" 3", []),
    {ok,3} = db_ets:read(sune, Db0),
    {error,instance} = db_ets:read(sune, db_ets:delete(sune, Db0)),

    io:format(" 4", []),			%Delete non-existent key
    {error,instance} = db_ets:read(sten, Db0),
    {error,instance} = db_ets:read(sten, db_ets:delete(sten, Db0)),
...
insert_delete_all() ->
    ...
    io:format(" 1", []),
    Empty = lists:foldl(fun (K, D) -> db_ets:delete(K, D) end,
			Db0, [curt,bert,sune]),
    io:format(" 2", []),
    Empty = lists:foldl(fun (K, D) -> db_ets:delete(K, D) end,
			Db0, [bert,sune,curt]),
...
insert_overwrite_one() ->
    ...
    io:format(" 1", []),
    {ok,1} = db_ets:read(curt, Db0),
    {ok,10} = db_ets:read(curt, db_ets:write(curt, 10, Db0)),

    io:format(" 2", []),
    {ok,2} = db_ets:read(bert, Db0),
    {ok,20} = db_ets:read(bert, db_ets:write(bert, 20, Db0)),

    io:format(" 3", []),
    {ok,3} = db_ets:read(sune, Db0),
    {ok,30} = db_ets:read(sune, db_ets:write(sune, 30, Db0)),
...
insert_overwrite_all() ->
    ...
    io:format(" 1", []),
    {ok,10} = db_ets:read(curt, Db1),

    io:format(" 2", []),
    {ok,20} = db_ets:read(bert, Db1),

    io:format(" 3", []),
    {ok,30} = db_ets:read(sune, Db1),

    io:format(" 4", []),			%Read non-existent key
    {error,instance} = db_ets:read(sten, Db1),
    io:format(" - ok\n").

%% init_db([{Key,Val}]) -> Database.
%%  Create and initialise a database with keys curt, bert and sune.

init_db(Vals) -> write_db(Vals, db_ets:new()).

%% write_db([{Key,Val}], Database) -> Database.
%%  Writes in all key-val into existing database.

write_db(Vals, Db) ->
    lists:foldl(fun ({K,V}, D) -> db_ets:write(K, V, D) end, Db, Vals).
